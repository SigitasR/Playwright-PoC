...
export class BarboraSpecialOfferSection {
    ...
    async checkIfAllProductCardDetailsAreDisplayed() {
        ...
        for (let i = 0; i < count; i++) {
            let productWrapper = await this.specialOfferWrapper.locator('div.b-product--wrap2').nth(i)
    ...
    async clickSpecialOffer(offerNumber = 0) {
        ...
        let offerCount = await this.specialOfferWrapper
            .locator('div.b-product--wrap2')
            .count()

        const click = offerNumber < offerCount ? offerNumber : 0

        await this.specialOfferWrapper
            .locator('div.b-product--wrap2')
            .nth(click)
            .locator('img')
            .click()
